...
if GOOGLE_API_KEY:
    ...
    # FIX: Corrected model name to prevent 404 "Not Found" errors in AI generation
...
async def translate_ai(req: TranslationRequest, user_id: str = Query(...)): 
    ...
    try:
        ...
        prompt += (
            "\n🚨 CRITICAL MEAN REVERSION OVERRIDE:\n"
            "If the technical ledger indicates 'MACD Divergence' is 'Negative' OR 'Options Flow' is 'Put Heavy', you are explicitly FORBIDDEN from issuing a 'BUY' or 'STRONG BUY' signal, regardless of how high the Quant Score or Fundamental data is.\n"
            "Instead, you MUST issue a 'WAIT FOR MEAN REVERSION' signal. You must explain that despite strong fundamentals, momentum is breaking down and smart money is hedging (buying puts), making the current price a high-risk bull trap. Anchor your target entry to the Mathematical Floor.\n\n"
        )
        ...
        prompt += (
            ...
            "### ⚖️ AI SIGNAL: [BUY/HOLD/TRIM/SELL/WAIT FOR MEAN REVERSION]\n"
