...
export default function Home() {
  ...
  // 🚨 RESTORED: ADD TO VAULT FUNCTION
  const addToVault = async () => {
    ...
    if (error) alert("Vault Error: " + error.message);
        ...
        alert(`${confirmedTicker} secured to vault.`);
        fetchWatchlist(user.id);
    }
  };
    ...
    try {
        const res = await fetch(`${BACKEND_URL}/translate`, {
            ...
            body: JSON.stringify({ 
                ...
                data_context: { 
                    score: data.score, price: data.price, stance: data.holding_analysis?.status,
                    fundamentals: data.fundamentals, news_titles: data.news?.map((n:any)=>n.title).join(" | ") 
                } 
            })
        });
        const result = await res.json();
  ...
  if (!user) return (
    <main className="min-h-screen bg-[#020617] flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-slate-900/40 border border-slate-800 p-10 rounded-[48px] shadow-2xl text-center">
        <h1 className="text-4xl font-black text-white mb-8 tracking-tighter">TRADEBOTICS<span className="text-blue-500">AI</span></h1>
  ...
  if (!hasAcceptedTerms) return (
    <main className="min-h-screen bg-[#020617] flex items-center justify-center p-6">
      <div className="w-full max-w-lg bg-[#0f172a] border border-slate-800 rounded-[32px] p-10 shadow-2xl relative">
        <h2 className="text-xl font-bold text-white mb-8">⚖️ Terms of Service & Legal Disclaimer</h2>
        <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-4 custom-scrollbar">
            ...
            <div><p className="text-white font-black text-sm mb-2">1. No Financial Advice</p><p className="text-slate-400 text-[13px] leading-relaxed">TradeBotics AI is strictly an educational tool. We are not registered financial advisors.</p></div>
            <div><p className="text-white font-black text-sm mb-2">2. Assumption of Risk</p><p className="text-slate-400 text-[13px] leading-relaxed">Trading involves high risk. You may lose your initial investment.</p></div>
            <div><p className="text-white font-black text-sm mb-2">3. Limitation of Liability</p><p className="text-slate-400 text-[13px] leading-relaxed">TradeBotics AI bears no legal responsibility for financial losses.</p></div>
            <div><p className="text-white font-black text-sm mb-2">4. Data Accuracy</p><p className="text-slate-400 text-[13px] leading-relaxed">Market data is inherently volatile and information may be delayed.</p></div>
        </div>
    ...
    <main className="min-h-screen bg-[#020617] text-slate-300 p-6 flex flex-col font-sans relative">
      ...
      {deepDiveResult && (
        <div className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="w-full max-w-2xl max-h-[90vh] bg-slate-900 border border-blue-500/30 p-10 rounded-[48px] shadow-2xl flex flex-col">
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-500 mb-8">AI Deep Dive</p>
              <div className="overflow-y-auto flex-1 mb-10"><p className="text-white text-lg font-medium italic whitespace-pre-wrap">"{deepDiveResult}"</p></div>
              <button onClick={() => setDeepDiveResult(null)} className="w-full bg-slate-800 py-4 rounded-2xl font-black text-[10px] uppercase">Close Briefing</button>
            </div>
        </div>
      )}
      ...
      {selectedArticle && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
            <div className="w-full max-w-3xl max-h-[85vh] bg-[#020617] border border-blue-500/30 rounded-[40px] flex flex-col">
              <div className="p-8 border-b border-slate-800 flex justify-between">
                <div><p className="text-[10px] font-black uppercase text-blue-500 mb-2">AI Synthesis</p><h2 className="text-2xl font-bold text-white">{selectedArticle?.title}</h2></div>
              ...
              <div className="p-8 bg-slate-950 overflow-y-auto flex-1">
                 {isSummarizing ? <div className="animate-pulse h-4 bg-slate-800 rounded w-full" /> : <div className="space-y-6">{selectedArticle?.summary?.map((p: string, i: number) => <p key={i} className="text-slate-300 leading-relaxed">{p}</p>)}</div>}
              </div>
            </div>
        </div>
      )}
      ...
      <div className="flex justify-between items-center mb-12">
        ...
        <Link href="/portfolio" className="hidden lg:flex items-center gap-3 px-6 py-3 bg-slate-900/50 border border-slate-800 rounded-full hover:border-blue-500/50 transition-all group">
        ...
        <div className="flex gap-3 bg-slate-900/80 p-3 rounded-[24px] border border-slate-800">
          <input value={ticker} onChange={(e) => setTicker(e.target.value.toUpperCase())} onKeyDown={(e) => e.key === 'Enter' && runAnalysis()} className="bg-transparent text-white font-black w-48 px-4 outline-none text-lg" placeholder="TICKER..." />
          ...
          <button onClick={handleSignOut} className="text-[9px] border border-slate-800 px-5 rounded-full uppercase font-black text-slate-400">Logout</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-8 flex-1">
        <div className="col-span-12 lg:col-span-3 space-y-8">
          {data && (
            <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-8 shadow-inner relative group">
              {/* 🚨 RESTORED: ADD TO VAULT BUTTON */}
              <button onClick={addToVault} className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 bg-blue-600 hover:bg-blue-500 text-white text-[9px] font-black px-4 py-2 rounded-full transition-all">ADD TO VAULT</button>
          ...
          {data?.fundamentals && (
            <div className="bg-[#020617] border border-blue-500/20 rounded-[40px] p-10 shadow-[inset_0_0_20px_rgba(59,130,246,0.05)]">
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-8">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 text-center">Secured Watchlist</p>
            <div className="space-y-3 max-h-[350px] overflow-y-auto custom-scrollbar pr-2">
              {watchlist?.map((item, i) => (<button key={i} onClick={() => runAnalysis(item.ticker)} className="w-full flex justify-between p-4 rounded-2xl bg-slate-950 border border-slate-800 hover:border-blue-500/50 transition-all"><span className="font-black text-white text-lg">{item.ticker}</span><span className="text-[10px] font-bold text-slate-500">{item.score} PTS</span></button>))}
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-6 flex flex-col gap-8">
          {!data ? (
              <div className="flex flex-col gap-8 h-full"><div className="bg-slate-900/30 border border-slate-800 p-10 rounded-[48px] text-center font-bold text-white uppercase tracking-widest">Market Pulse</div><MarketScreener /></div>
          ) : (
            <>
                ...
                {data?.ledger?.map((item: any, i: number) => (
                  <div key={i} className="bg-slate-900/30 border border-slate-800/50 p-8 rounded-[40px] flex justify-between items-start transition-all hover:border-slate-600">
                    <div className="flex-1">
                        ...
                        <p className="text-slate-400 text-sm italic border-l-2 border-slate-800 pl-4 font-medium">"{item.reasoning}"</p>
                    </div>
                  </div>
                ))}
              </div>
              <button onClick={() => runDeepDive('strike_zone')} className="w-full bg-blue-600 hover:bg-blue-500 py-5 rounded-[32px] font-black text-xs uppercase tracking-[0.3em] transition-all">🎯 Calculate Entry Zone</button>
            </>
          )}
        </div>
        ...
        <div className="col-span-12 lg:col-span-3 space-y-8">
          <div className="bg-slate-900 border border-slate-800 rounded-[40px] p-10 shadow-2xl">
             ...
             {data ? (
                <>
                  ...
                  <button onClick={() => runDeepDive('verdict')} className="w-full mb-4 bg-blue-600/10 border border-blue-500/20 py-3 rounded-xl text-blue-500 font-black text-[10px] uppercase">Execute Verdict</button>
                  <div className="p-5 bg-blue-500/5 border-l-2 border-blue-500 rounded-r-2xl min-h-[50px]"><p className="text-slate-200 text-sm font-bold italic">"{data?.ai_tactical}"</p></div>
                  
                  <div className="mt-10 p-8 bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-[32px] shadow-2xl relative overflow-hidden group">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Portfolio Intelligence</p>
                    <h3 className="text-white font-black text-xl mb-3">Tailor Your AI.</h3>
                    <p className="text-slate-400 text-[11px] mb-8 leading-relaxed">Sync holdings for personalized exit strategies and cost-basis mapping.</p>
                    <Link href="/portfolio"><button className="w-full py-4 bg-slate-800 text-[10px] font-black uppercase rounded-xl hover:bg-blue-600 transition-all text-white border border-slate-700">Sync Portfolio Data</button></Link>
                  </div>
                </>
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-8 flex flex-col h-[600px] overflow-hidden shrink-0">
             ...
             <button onClick={() => runDeepDive('sentiment')} className="w-full mb-6 bg-blue-600/10 border border-blue-500/20 py-4 rounded-2xl text-blue-500 font-black text-[10px] uppercase tracking-widest">🌐 Global Sentiment Check</button>
             <div className="space-y-4 overflow-y-auto flex-1 custom-scrollbar">
                {newsToDisplay.map((item: any, i: number) => (
                    <div key={i} onClick={() => handleArticleClick(item)} className="bg-slate-950 border border-slate-800 p-5 rounded-3xl cursor-pointer hover:border-blue-500/50 transition-all">
                        <p className="text-sm font-bold text-slate-200 leading-snug line-clamp-3">{item.title}</p>
                        <div className="flex justify-between items-center mt-4 pt-4 border-t border-slate-800/30">
                            ...
                            <span className="text-[8px] bg-blue-600/10 text-blue-500 px-2 py-0.5 rounded-full font-black uppercase">AI Synthesis</span>
                        </div>
                    </div>
                ))}
             </div>
