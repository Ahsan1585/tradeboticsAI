...
export default function Home() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      setUser(user);
      if (user) { fetchWatchlist(user.id); fetchGlobalNews(); }
    };
    checkUser();
  }, []);
  ...
  const handleAcceptTerms = async () => {
    ...
    if (!error) setHasAcceptedTerms(true);
  };
  ...
  const handleSignOut = async () => { await supabase.auth.signOut(); setUser(null); setHasAcceptedTerms(false); setData(null); };
    ...
    <main className="min-h-screen bg-[#020617] text-slate-300 p-6 flex flex-col font-sans relative">
      ...
      {deepDiveResult && (
        <div className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
           <div className="w-full max-w-2xl max-h-[90vh] bg-slate-900 border border-blue-500/30 p-10 rounded-[48px] shadow-2xl flex flex-col overflow-hidden">
              ...
              <button onClick={() => setDeepDiveResult(null)} className="w-full bg-slate-800 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:text-white transition-colors shrink-0">Close Briefing</button>
           </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-12 gap-8 flex-1">
        ...
        <div className="col-span-12 lg:col-span-3 space-y-8">
             ...
             {data ? (
                <>
                  ...
                  <div className="p-8 bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-[32px] shadow-2xl relative overflow-hidden group">
                    ...
                    <p className="text-slate-400 text-[12px] leading-relaxed mb-8">
                      Upload your current portfolio data to unlock personalized exit strategies, risk-adjusted target trims, and accurate cost-basis mapping.
                    </p>
                    <Link href="/portfolio">
                      <button className="w-full py-4 bg-slate-800 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-600 transition-all text-white border border-slate-700 hover:border-blue-500 shadow-lg">
                        Sync Portfolio Data
                      </button>
                    </Link>
                  </div>
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-8 h-[600px] flex flex-col h-full overflow-hidden shrink-0">
             <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-6 text-center text-slate-500">AI Intelligence Wire</p>
             ...
             <div className="space-y-4 overflow-y-auto custom-scrollbar flex-1">
                {newsToDisplay.map((item: any, i: number) => (
                    <div key={i} onClick={() => handleArticleClick(item)} className="bg-slate-950 border border-slate-800 p-5 rounded-3xl cursor-pointer hover:border-blue-500/50 group transition-all">
                        <p className="text-sm font-bold text-slate-200 group-hover:text-blue-400 leading-snug line-clamp-3 transition-colors">{item.title}</p>
