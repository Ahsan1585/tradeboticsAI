...
// 🚨 PRODUCTION URL (Hardwired to eliminate PNA security pop-ups)
...
export default function Home() {
  ...
  if (!hasAcceptedTerms) return (
    <main className="min-h-screen bg-[#020617] flex items-center justify-center p-6">
      <div className="w-full max-lg bg-[#0f172a] border border-slate-800 rounded-[32px] p-10 shadow-2xl relative">
      ...
      {deepDiveResult && (
        <div className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="w-full max-w-2xl max-h-[90vh] bg-slate-900 border border-blue-500/30 p-10 rounded-[48px] shadow-2xl flex flex-col overflow-hidden">
              ...
              <div className="overflow-y-auto flex-1 mb-10 pr-2"><p className="text-white text-lg font-medium leading-relaxed italic whitespace-pre-wrap">"{deepDiveResult}"</p></div>
              <button onClick={() => setDeepDiveResult(null)} className="w-full bg-slate-800 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest">Close Briefing</button>
            </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-12 gap-8 flex-1">
        ...
        <div className="col-span-12 lg:col-span-3 space-y-8">
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-8 shadow-inner">
            ...
            <div className="space-y-3 max-h-[350px] overflow-y-auto custom-scrollbar pr-2">
              {watchlist?.map((item, i) => (<button key={i} onClick={() => runAnalysis(item.ticker)} className="w-full flex justify-between p-4 rounded-2xl bg-slate-950 border border-slate-800 hover:border-blue-500/50 group transition-all"><span className="font-black text-white text-lg">{item.ticker}</span><span className="text-[10px] font-bold text-slate-500">{item.score} PTS</span></button>))}
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-3 space-y-8">
          <div className="bg-slate-900 border border-slate-800 rounded-[40px] p-10 shadow-2xl">
             ...
             {data ? (
                <>
                  ...
                  {/* 🚨 RESTORED: AI VERDICT BUTTON STYLE & HOVER */}
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-8 flex flex-col h-[600px] overflow-hidden shrink-0">
             ...
             {/* 🚨 RESTORED: GLOBAL SENTIMENT BUTTON STYLE & HOVER */}
             <button onClick={() => runDeepDive('sentiment')} className="w-full mb-6 bg-blue-600/10 border border-blue-500/20 py-4 rounded-2xl text-blue-500 font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all shadow-blue-500/5">
