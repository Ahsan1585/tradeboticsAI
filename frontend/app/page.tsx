...
export default function Home() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      // 🚨 UPDATED: Clear instructions for email verification
      if (error) showToast(error.message); 
      else showToast("Registration received. Please check your email to verify your account.");
    } else {
      const { data, error } = await supabase.auth.signInWithPassword({ email, password });
      if (error) {
          // 🚨 UPDATED: Custom intercept for unverified emails to make it look professional
  ...
  const addToVault = async () => {
    ...
    if (error) showToast("Vault Error: " + error.message);
    else {
        showToast(`${confirmedTicker} SECURED TO VAULT`);
        fetchWatchlist(user.id);
    }
  };

  const removeFromVault = async (removeTicker: string, e: React.MouseEvent) => {
    e.stopPropagation(); 
    if (!user) return;
    ...
    if (error) {
        ...
        showToast(`${removeTicker} PURGED FROM VAULT`);
        fetchWatchlist(user.id);
    }
  };
            ...
            <div className="space-y-3 max-h-[350px] overflow-y-auto custom-scrollbar pr-2">
              {watchlist?.map((item, i) => (
                <div key={i} className="flex gap-2 w-full group">
                  ...
                  <button onClick={(e) => removeFromVault(item.ticker, e)} className="px-4 rounded-2xl bg-slate-950 border border-slate-800 hover:bg-red-500/10 hover:border-red-500 hover:text-red-500 text-slate-600 transition-all font-black text-xs">✕</button>
                </div>
              ))}
            </div>
          </div>
